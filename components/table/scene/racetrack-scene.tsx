 ...
 *
 * The difference from `table-scene.tsx` is which layer is in charge, worth
 * stating plainly since the two files otherwise look alike.
 *
 ...
 * only holds the far rail and the opponents' heads up out of the strip.
 *
 * Smaller than the review render's 0.2, because that render framed a whole
 * viewport while this canvas fills `.table-area` only. The action bar is a
 * sibling below it (`.action-layer`, 06-table.css) and already sits outside
 * this box, so reserving its height again here would count it twice and
 * push the crowd up by a strip that isn't there.
 */
...
export interface RacetrackLayout {
  ...
  seats: Array<{
     ...
     * amount label needs this rather than `toward`, which is a direction for
     * things that travel a CSS-tuned distance; the classic room's own
     * `--bet-reach-*` constants (08-seat.css) are tuned against that room's
     * ellipse, and reusing them here landed the label off the actual pile by
...
export function RacetrackScene({
  ...
  useEffect(() => {
    ...
    const fit = () => {
      ...
      // the scale everything is *laid out* against, so the mound's columns
      // and the chips filling them agree about how big a chip is.
     ...
     * only the chips. Worth stating, since the opposite is the natural
     * assumption: a canvas keeps its pixels when nobody draws to it, so a
     * stopped loop leaves the table on screen exactly as it was. What a
     * stopped loop cannot do is notice that something needs redrawing,
     * hence `pump` on every event that changes the picture, `fit` included.
     *
     * One thing not to copy from here: the room is repainted in full on
     * every frame, where the classic room clears to transparent and paints
     * only chips. That is the price of owning the table, paid only while
