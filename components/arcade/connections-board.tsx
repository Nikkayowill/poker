  ...
  return (
    <main className="bj-shell puzzle-shell">
      {/* floor-bar/floor-head/duel-panel are the same entry-screen furniture
          ...
          instead of two. Only the pre-game step borrows it; the board below
          stays this file's own, since a daily puzzle's share/countdown
          features have no equivalent there. */}
      ...
      {/* The puzzle-number/player strip the header used to hold, kept for
          ...
          but there's no clock here, just groups solved. */}
      {round && profile && (
        <div className="puzzle-player">
          ...
          <span className="bj-hand-who">
            ...
            <span className="bj-hand-caption">
              {meta ? `Puzzle #${meta.puzzleNumber}` : ""} · {round.revealed.filter((group) => group.solved).length}/4 groups
            </span>
          </span>
        </div>
      )}
