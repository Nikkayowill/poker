/**
 * The exchange window: the one place Gold ever leaves the Homestead.
 *
 ...
 * never decides how big the bucket is. If a change makes `HOMESTEAD_GOLD_CEILING`
 ...
 * below the ~7,500/day the pre-Bushels Homestead paid out with no cap at all.
 *
 ...
 */
export const HOMESTEAD_GOLD_CEILING = 5_000;
 ...
 */
export const HOMESTEAD_GOLD_PER_BUSHEL = 2;
 ...
 */
export const HOMESTEAD_MAX_EXCHANGE_BUSHELS = Math.ceil(
  HOMESTEAD_GOLD_CEILING / HOMESTEAD_GOLD_PER_BUSHEL,
);

/** What exchanging `bushels` pays, in Gold. */
export function goldForBushels(bushels: number): number {
  return bushels * HOMESTEAD_GOLD_PER_BUSHEL;
}
...
export function bushelsWithinAllowance(remainingGold: number): number {
  return Math.max(0, Math.floor(remainingGold / HOMESTEAD_GOLD_PER_BUSHEL));
}
 ...
 */
export function homesteadExchangeDay(now: Date): string {
...
export interface HomesteadExchangeState {
...
export function exchangeState(usedToday: number, now: Date): HomesteadExchangeState {
  const used = Math.min(Math.max(0, usedToday), HOMESTEAD_GOLD_CEILING);
  const remaining = HOMESTEAD_GOLD_CEILING - used;
  return {
    rate: HOMESTEAD_GOLD_PER_BUSHEL,
    ceiling: HOMESTEAD_GOLD_CEILING,
    usedToday: used,
    remaining,
