 ...
 * array. PlayerSeat is memoised with a hand-written comparator, and a fresh
 * collection every render either defeats the memo entirely or, if it's left
 * out of the comparator, silently freezes on whatever the seat first saw. A
 * string compares by value with `===`, so neither trap is available.
 *
