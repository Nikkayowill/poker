 ...
 * contract for what replaced it: one room, built as separate 2D layers at
 * known depths, with the live game rendered in the DOM on top.
 *
 ...
 *
 * The dealers sit behind the table, not in front of it. They used to be in
 * front so their paws could drape over the rail, which worked while they were
 * a flat CSS crop but broke the moment photographic art landed; see the note
 * in app/styles/27-dealer-stage.css for why occlusion is what sells the depth.
 *
 * Separate layers rather than one flattened image, because a single baked
 * render can't swap an expression, can't let a card pass in front of the
 * rail, and would need re-baking the whole room every time a dog's mouth
 * changes. Four files that composite at runtime cost the same bandwidth and
 * buy every one of those.
 *
 * Every `src` below may be null, and null isn't an error state, it's "no
 * verified asset yet" -- the same convention lib/audio/music-manifest.ts uses
 * for an empty playlist and `status: "coming-soon"` uses in
 * lib/arcade/games.ts. The component paints a CSS placeholder for any layer
 * with no file, so the page is complete before a single PNG exists and each
 * asset can land on its own. Dropping a file at the documented path and
 * flipping one constant is the whole remaining step per layer.
 *
 * This lives in lib/ because vitest.config.ts only collects lib/ and app/,
 * and the expression mapping below has to be total over every phase and
 * outcome the engine can produce -- a hole in it is two dogs frozen mid-grin
 * through a losing hand, worth catching without a browser.
 */
...
/**
 * What the dogs are doing: four drawings per dog and no more.
 *
 * Named for the moment each one serves rather than the face it wears, so the
 * art can be matched to the need instead of the need being bent to fit
 ...
 *
 * They cheer when the player wins and commiserate when the player loses, not
 * the house's way round. These two are dogs the player is meant to like; a
 * dealer that looks pleased while taking your stake is a different, meaner
 * joke than the one this feature is telling.
 *
...
export interface SceneArt {
  /**
   * The casino behind the table. Full bleed, and the only layer allowed to be
   * soft: it's the one thing in the picture nobody looks at, and a sharp
   * background competes with the cards for the same eye.
   */
  room: string | null;
  /**
   ...
   * dealer stands on soft 17. Felt print is not decoration, it's the same
   * promise the page header makes, six inches lower -- see lib/arcade/games.ts's
   * own header for the same class of mistake in a hub blurb.
   */
  table: string | null;
}
 ...
 * from the corners rather than keyed on black globally. That distinction
 * matters: Finn is a black dog in a black waistcoat and a black bow tie, and a
 * global key punches straight through all three. The safe fuzz was measured
 * rather than guessed (above ~3% the fill starts eating his tie and eyes), and
 * the cut was checked for interior holes and against a light background, where
 * a black fringe would show. Recut them that way or not at all.
 *
 * Only this one pose was drawn -- the choice was between shipping the real art
 * now with one expression or holding all of it back until eight files exist.
 * This is the former, and dogArt() below falls back here for any expression
 * without its own file.
 *
 * The consequence: the dogs don't change face yet. dealerExpression() is still
 * computed, still total over every phase and outcome, and still reaches the
 * DOM as `data-expression`, but with only a portrait behind it the pair hold
 * this one look through a win and a bust alike. Dropping `loki-cheer.webp`
 * beside these and adding its entry to DOG_ART is the whole step per
 * expression.
 */
 ...
 *
 * Empty today; the pair are drawn from DOG_PORTRAIT above. Adding art means
 * adding entries here. It's a partial record rather than a full one so the
 * pair can be drawn one expression at a time instead of needing all eight
 * files before any of them can ship.
 */
 ...
 *
 * This exists because of a specific near-miss: the reference art commissioned
 * for this table had "INSURANCE PAYS 2 TO 1" and "DEALER MUST HIT SOFT 17"
 * painted across the cloth. lib/arcade/blackjack.ts implements no insurance at
 * all, and the dealer stands on soft 17, so the felt would have promised two
 * rules the game doesn't have, one of them the exact opposite of the rule it
 * does have, six inches under a page header that states it correctly.
 *
 ...
 * changes the cloth in the same commit and the felt can't drift from the
 * engine even in principle.
 *
 * The table art must ship without printed rules: cloth, rail, shoe and
 * discard tray only. Anything painted on is a claim nothing can check.
 */
