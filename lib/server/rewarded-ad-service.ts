 ...
 *
 * Adsterra's publisher products are banners, interstitials and popunders. None
 * of them is a rewarded-video placement, and none of them offers a
 * server-to-server completion postback -- so there is no signal, anywhere, that
 * this server can verify to establish that an ad was watched. Any code that
 * claims otherwise is trusting `fetch` from a browser the player controls.
 *
 ...
 *      client-supplied timestamp anywhere in this file, deliberately.
 ...
 *
 * ORDERING RULES, for the same reason blackjack-service.ts restates its own
 * rather than referencing them -- breaking one is a silent money bug:
 *
