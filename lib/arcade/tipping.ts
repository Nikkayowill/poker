 ...
 * button.
 *
 * What a tip actually does: the Gold leaves the wallet, permanently, to the
 * house; it earns progression XP at the ordinary rate, through the same
 * awardWager hook a wager uses; and the pair react, thanking the player by
 * name of amount.
 *
 * That XP is not a new curve, and it's worth stating why it isn't an
 * exploit. XP is Gold-wagered / 10 everywhere in this app. A player who
 * grinds XP by *playing* gives up the arcade's ~3% edge; a player who grinds
 * it by *tipping* gives up 100%. Tipping is the strictly worst way to buy a
 * level that exists, which is exactly what stops it becoming the efficient
 * one, the same argument lib/server/hand-completion.ts makes about chips and
 * Gold being at parity across the buy-in boundary.
 *
 * When it appears is the other half, and it's why this file exists rather
 * than a boolean in the component. An ask that's on screen from the first
 ...
 * they've either paid or waved it away. All of it is a pure function of
 * counters, so `npm test` can walk a whole session and assert the pair are
 * never nagging.
 *
 * The cadence is client state and the debit is server state. How often a
 * bubble appears is presentation and costs nothing if a refresh forgets it;
 * how much Gold left the wallet is not, and lives in
 ...
 * What a tip can be.
 *
 * Small, and not off the STAKES_TIERS ladder: that rule governs stakes,
 * because a stake the ladder can't select is a price no button can charge.
 * A tip is not a wager. The smallest tier is 1,000 Gold, which against a
 * 1,000 Gold hand would be a tip equal to the bet, and nobody tips the
 ...
 *
 * Three gates. The round-live one matters beyond politeness: the tip button
 * sits on the same felt as Hit and Stand, and a control that appears between
 * a player's eye and the Stand button mid-hand is a misclick waiting to
 * charge them for it.
 */
...
/*
 * There is no TIP_PROMPT constant here. The ask is TIP_LINE in
 * lib/arcade/dealer.ts, "Tip the dealer!", the line the pair have always
 * had, and a second near-identical string here would be two places to
