...
/** Same row-to-id mapping `listOwnedStackAcresPerks` runs below, pulled out
 *  for the batch RPC path -- the batch's own SQL doesn't filter `quantity >
 *  0` (unlike this reader's own `.gt("quantity", 0)`), so that check moves
 *  here instead. */
export function stackAcresOwnedPerksFromBatchRows(rows: { item_id: string; quantity: number | string }[]): string[] {
