...
interface StackAcresResponse {
  ...
  /** What a wheat plot's harvest and a Mill's output sit as. Absent from a
   *  response predating the processing track, which reads as an empty shelf. */
  inventory?: StackAcresInventory;
  /** The town's one open request, or null when there is not one. Optional for
   *  the same reason; `null` and "not sent" both mean nothing is posted. */
...
type Action =
  ...
  | { action: "upgrade-tool" }
  // Town Contracts. `request-contract` moves nothing; `fulfill-contract` is
  // one of the only two actions in the whole farm that PAY Gold out, and it
  // reserves against the same flat daily ceiling a harvest does. See
  // lib/server/stackacres-service.ts's header.
...
export function StackAcresFarm() {
  ...
  const [inventory, setInventory] = useState<StackAcresInventory>(() => emptyInventory());
  const [contract, setContract] = useState<StackAcresContractRow | null>(null);
  ...
  const applyResponse = useCallback((data: Partial<StackAcresResponse>) => {
    ...
    // `contract` is the one field here whose NULL is meaningful -- it is how
    // a settled contract clears off the board -- so this tests for the key
    // rather than for truthiness the way every line above it does.
    if ("contract" in data) setContract(data.contract ?? null);
