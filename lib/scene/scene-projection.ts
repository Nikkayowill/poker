 ...
 * falls off with depth, which is the entire point of it, since an
 * orthographic tilt cannot produce depth at any angle.
 *
 ...
 *
 * Both cameras happen to give `groundSquash` the same closed form, the sine
 * of the elevation. That's not a coincidence: it falls out of the ground
 * plane's angle to the view direction either way, and only the perspective
 * one also varies its scale with depth.
 */
...
export function orthographicProjection(view: SceneView): SceneProjection {
  return {
    ...
    // Constant, but still a function on purpose: a caller that reached for
