...
import { MIN_ANTE_UP_WAGER } from "@/lib/arcade/brain-word-guess";
...
export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-word-guess:read", 120, 60 * 1000);
  if (limited) return limited;

  const token = readOrCreateSessionToken(request);
  try {
    return withRequestSessionCookie(request, NextResponse.json(await readBrainWordGuess(token)), token);
  } catch (error) {
...
export async function POST(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-word-guess:start", 30, 60 * 1000);
  if (limited) return limited;
  ...
  try {
    ...
    const result = await openBrainWordGuess(token, parsed.data.wager);
