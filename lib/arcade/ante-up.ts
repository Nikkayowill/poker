 ...
 * outcome is whether the grid gets solved before the deadline, keeping it on
 * the skill side of the app's skill-vs-chance line.
 *
 * This is the whole game: there is no separate free daily Sudoku with its own
 * once-a-day gate. It's always wager-or-free and unlimited, and that's what
 * /games/sudoku renders directly.
 *
 ...
 * never leaves the server" contract via `toSudokuSnapshot`. The one thing this
 * file changes about how that engine is used is that `generateSudoku` is
 * seeded with a fresh random string per attempt rather than a calendar day, so
 * every attempt is its own grid.
 */
...
export interface AnteUpAttempt {
  ...
  wager: number;
  /** Copied from ANTE_UP_TIERS at start, not re-read at settlement, so a retuned table can't change a live attempt's payout underneath it. */
  multiplier: number;
  sudoku: SudokuRound;
  status: AnteUpStatus;
  startedAt: string;
 ...
 * the same contract DuelGame#tick documents and for the same reason: the poll
 * calls this on every read, and a fresh object on an attempt that isn't
 ...
 *
 * `sudoku.solution` is gone, not blanked but absent, same as toSudokuSnapshot.
 * That matters most of anything in this file: with the solution in hand,
 * "beat the clock" is a formality.
 */
