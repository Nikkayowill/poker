 ...
 *
 *   THE CROP FIELDS are the north half of the Homestead. `CROP_FIELD_BEDS` is
 *   32 by 32 soil tiles, and the map lays them out tile for tile with the
 *   field's top-left world corner at map pixel FIELD_ORIGIN. A bed the server
 *   stores at world tile (tx, ty) is drawn at exactly one map tile, and a tap
 *   on a map tile names exactly one world tile, so nothing about placement
 *   rules moves. art/stackacres-td/areas/rig/homestead.py draws the field at
 *   the same spot. They used to be a scene of their own ("oldfields") behind a
 *   fallen log; they are one map now, so a bed is a bed wherever it is dug.
 *
 *   THE HOMESTEAD STARTER BEDS (lib/stackacres/soil.ts's `homeStarterSoilTiles`)
 *   are their own small lattice, pinned the same way at `HOME_BEDS_ORIGIN` --
 *   they are not Crop Fields ground, so they do not share `FIELD_ORIGIN`, and a
 *   farm's soil map can hold tiles on either lattice depending which one a
 *   tile's (tx, ty) falls in.
 *
...
/** Map pixel of the Crop Fields' top-left world corner (map tile 6, 2). The
 *  fields sit at the top of the map, so this did not move when they stopped
 *  being a scene of their own -- everything below them did. */
...
export function worldToMap(world: WorldPoint): MapPoint | null {
  if (inCropField(world)) return { area: "homestead", ...fieldWorldToMap(world) };
 ...
 * hold the six free starter beds in the west one). This is not Crop Fields
 * ground, so it gets its own origin instead of `FIELD_ORIGIN` even though both
 * are now on the same map.
 *
 * `HOME_BEDS_ORIGIN` is the map pixel of soil tile `HOME_STARTER_ORIGIN`, the
 * west paddock's top-left square: farmyard tile (4, 15) plus `HOME_SHIFT`, the
 * same corner the rig's WEST_BED starts on
 * (art/stackacres-td/areas/rig/homestead.py). The east paddock is on the same
 * lattice, so it needs no origin of its own -- it is simply further along.
 */
const HOME_BEDS_ORIGIN = { x: 64, y: 240 + HOME_SHIFT } as const;
