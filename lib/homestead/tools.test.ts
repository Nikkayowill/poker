...
import { HOMESTEAD_CATALOGUE, HOMESTEAD_FIELD_CAP, HOMESTEAD_PEN_CAP } from "./catalogue";
...
describe("the caps", () => {
  const working = (stock: "sprout" | "hen") =>
    plot("working", { stock, stake: HOMESTEAD_CATALOGUE[stock].seedCost });
  ...
  it("blocks a full pen track without touching the field track", () => {
    const plots = Array.from({ length: HOMESTEAD_PEN_CAP }, () => working("hen"));
    expect(
  ...
  it("blocks a full field track without touching the pen track", () => {
    const plots = Array.from({ length: HOMESTEAD_FIELD_CAP }, () => working("sprout"));
    expect(
      affordanceFor("plant", plot("empty"), context({ plots, selectedStock: "sprout" })).kind,
    ).toBe("blocked");
    expect(
