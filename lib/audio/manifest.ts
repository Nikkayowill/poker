 ...
 * how loud it sits in the mix. The only place a filename should appear;
...
export const SOUND_FILES: Record<SoundEffect, string | null> = {
  ...
  // The flop reveals three cards at once, so it gets the richer flip sound
  // to mark the bigger moment rather than reusing the routine deal sound.
  ...
  // The stock crowd cheer, reserved for a genuinely huge pot (see
  ...
  // sound like a stadium" complaint this split fixes. Still 2.5s, since it
  // has to finish reacting before the next hand deals.
  ...
  // export; trimmed to that, at 1.85s, it clears NEXT_HAND_DELAY_MS the same
  // as the cheer. (Its sibling `Winning_a_regular_pot.mp3` has a genuine
  // 5.2s hit and still doesn't fit, so it's left on disk unused.)
 ...
 *
 * These are measurements, not preferences. The assets come from four stock
 * libraries plus the owner's own set and span 15.3 dB of mean level (the
 * crowd cheer at -16.3 against the epic card flip at -31.6), all played at
 * volume 1.0, so the mix was whatever each source happened to normalise to.
 * That's why the room roared and the flop was a whisper.
 *
 ...
 * judgement rather than measurement: the table's balance written down,
 * loudest moment to quietest housekeeping.
 *
 * A gain can only ever attenuate (HTMLMediaElement.volume is capped at 1),
 * so every target here must be at or below its file's measured level, and
 * the unit tests enforce exactly that. The practical consequence is that the
 * quietest asset sets the ceiling for its own event: the flop runs wide open
 * at unity and is still the most restrained of the big moments. That's an
 * asset limitation, not a tuning choice. If the flop should hit harder, the
 * file has to be replaced; no number in this file can do it.
 */
const EFFECT_TARGET_DB: Record<SoundEffect, number> = {
  ...
  // shorter than the cheer above, a moment rather than a celebration.
  "win-modest": -26,
  // The bets, in the order they deserve attention.
  "all-in": -26,
  raise: -29,
  call: -31,
  // Chips moving generally. The quietest of the chip sounds because it
  // layers *underneath* call/raise in the same snapshot: texture for the
  // stack movement, not the announcement of a decision.
  chips: -36,
  ...
  ui: -36,
  // A choice is worth hearing slightly more than a navigation tap, since it
