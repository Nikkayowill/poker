 ...
 * unordered (a Corn row can cross a Sunflower row from either side, and so
...
/**
 * Every crossbreedable pair StackAcres actually grows. Real StackAcres stock
 * only (see ./catalogue.ts), the same rule ./juice.ts's own header states for
 * its shard styling, and it applies just as much to what two rows are
 * allowed to cross.
 *
 * Three tracks, in the order the odds fall: two crops standing together is
 * the easiest, most literal reading of "cross-pollination" and gets the
 * richest chance; one animal beside one crop sits in the middle; two kinds
 * of livestock standing together is the most far-fetched pairing here and
 * gets the smallest. Every hybrid in ./crossbreed-items.ts is bred by
 * exactly one pair below, and crossbreeding.test.ts holds that. Nothing
 ...
 * (2026-09-12) -- `corn`/`wheat`/`carrot`/`potato` here are the ids that
 * survived or replaced the 22-crop CraftPix roster's own pairings
 * (`sunflower` and `beet`, both gone, are what `wheat` and `potato`
 * stand in for below).
 */
export const CROSSBREED_MATRIX: readonly CrossbreedMatrixEntry[] = [
  { a: "corn", b: "wheat", hybrid: "golden_maize", chance: 0.18 },
  { a: "carrot", b: "potato", hybrid: "candied_husk", chance: 0.15 },
  { a: "hen", b: "wheat", hybrid: "sunroot_egg", chance: 0.1 },
 ...
 *  from ./catalogue.ts's roster, and by the bed's own planting picker, which
 *  only offers stock that can ever cross with something: planting anything
 *  else in a bed that pays nothing on a plain harvest is a pure loss. */
...
/**
 * One bed cell as the client renders it: the stored row plus its readiness,
 * derived ONCE here by the server's own clock -- the same "server decides
 * readiness, the client only displays it" posture StackAcresUnitSnapshot
 * takes. `startedAt`/`readyAt` ride along so the sheet can draw a countdown
 * between polls without a second derivation of its own.
 */
export interface CrossbreedPlotView extends StackAcresCrossbreedPlotRow {
  startedAt: string;
  ready: boolean;
}

/** The Crossbreeding Bed as StackAcresView carries it. */
export interface CrossbreedBedView {
  plots: CrossbreedPlotView[];
  /** Every hybrid bred so far. A missing key is 0, same convention every
   *  other inventory read in StackAcres uses. */
  inventory: Partial<Record<CrossbreedItem, number>>;
}

/** What one settled harvest tap came back as -- see
 *  lib/server/stackacres-crossbreeding-service.ts's harvestCrossbreedBed.
 *  Lives here rather than beside that function so the sheet can name it
...
export interface CrossbreedHarvestSettlement {
   ...
   *  null alongside a null `hybridItem`. Never the delta. */
  hybridQuantity: number | null;
}

export function toCrossbreedPlotView(
  row: StackAcresCrossbreedPlotRow & { startedAt: string },
  atMs: number,
): CrossbreedPlotView {
  return {
    id: row.id,
...
export function emptyCrossbreedBedView(): CrossbreedBedView {
  return { plots: [], inventory: {} };
}

/** The engine's own narrow grid off a view -- what a client-side "would
 *  this cross?" hint feeds `evaluateMutationChance`. `ready` is re-derived
 *  against `atMs` so a row that ripened since the last poll reads as ripe
 *  in the hint too; the server's own roll still decides for real. */
export function crossbreedGridFromView(bed: CrossbreedBedView, atMs: number): CrossbreedGridSnapshot {
  return bed.plots.map((plot) => ({
    id: plot.id,
    row: plot.row,
    col: plot.col,
    stock: plot.stock,
    ready: plot.ready || isCrossbreedPlotReady(plot, atMs),
  }));
}
