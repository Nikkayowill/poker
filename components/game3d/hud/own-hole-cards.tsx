 ...
 *
 * OFF THE FELT ON PURPOSE. This is a fixed HUD element beside the local
 * avatar and immediately above the action console (see .ownHandLayer in
 * game3d.module.css), not a projected 3D anchor like the board. The player's
 * own hand reads as something they're holding, not another prop lying on the
 * cloth, so it never has to fight the felt's perspective or share space with
 * a folding seat's recede.
 *
...
export function OwnHoleCards({ seats, live = false }: { seats: SeatModel[]; live?: boolean }) {
  const mine = seats.find((seat) => seat.isMine);
  if (!mine || !mine.inHand || mine.holeCards.length === 0) return null;

  return (
    <div className={`${styles.ownHandLayer} ${live ? styles.ownHandLayerLive : ""}`}>
      {mine.handLabel ? (
        <span className={styles.ownHandStrength} aria-live="polite">
          {mine.handLabel}
        </span>
      ) : null}
