...
import { WORDLE_ALLOWED_GUESSES, isAllowedWordleGuess } from "./wordle-dictionary";
import { WORDLE_WORD_LENGTH, startWordleRound } from "./wordle";
...
describe("WORDLE_ANSWERS", () => {
  it("is every answer a legal guess", () => {
    ...
    const orphans = WORDLE_ANSWERS.filter((word) => !isAllowedWordleGuess(word));
    expect(orphans).toEqual([]);
  });

  it("is all five-letter lowercase words", () => {
    const malformed = WORDLE_ANSWERS.filter((word) => !/^[a-z]{5}$/.test(word));
    expect(malformed).toEqual([]);
    expect(WORDLE_WORD_LENGTH).toBe(5);
  });

  it("has no duplicates", () => {
    // A duplicate silently doubles that word's odds and shortens the cycle.
    expect(new Set(WORDLE_ANSWERS).size).toBe(WORDLE_ANSWERS.length);
  });

  it("every answer opens a playable round", () => {
    expect(() => WORDLE_ANSWERS.forEach(startWordleRound)).not.toThrow();
  });
  ...
  it("runs long enough that a player will not see a repeat", () => {
    ...
    expect(WORDLE_ANSWERS.length).toBeGreaterThan(730);
  });
});

describe("WORDLE_ALLOWED_GUESSES", () => {
  it("is wide enough that real words are not rejected", () => {
    ...
    expect(WORDLE_ALLOWED_GUESSES.size).toBeGreaterThan(12_000);
  });

  it("accepts ordinary opening guesses", () => {
    ["slate", "crane", "adieu", "audio", "raise", "stare"].forEach((word) => {
      expect(isAllowedWordleGuess(word)).toBe(true);
    });
  });

  it("rejects a non-word and normalizes what it is given", () => {
    expect(isAllowedWordleGuess("zzzzz")).toBe(false);
    expect(isAllowedWordleGuess("  SLATE ")).toBe(true);
  });

  it("holds only five-letter lowercase words", () => {
    const malformed = [...WORDLE_ALLOWED_GUESSES].filter((word) => !/^[a-z]{5}$/.test(word));
    expect(malformed).toEqual([]);
  });
});
