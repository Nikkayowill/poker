...
// (see the note at the top of homestead-art.ts).
 ...
 * homestead-scene.ts from lib/homestead/props.ts's sizes, so it is not
...
export const PROP_PAINTERS: Record<PropPainterName, Painter> = {
  ...
  // scene (lib/homestead/props.ts's WINDMILL_HUB) so they can turn.
