 ...
 *
 *  Re-derived for the 2026-09-08 map restructure's hub-and-spoke layout by
 *  projecting each district's approach relative to the Farmstead's own and
 *  reading the screen angle off it -- `meadow` dropped out of this record
 *  the same restructure merged it into the Farmstead outright (see
 *  ./zones.ts's own header), and `oxfields` moved from "east" to "south-west"
 *  since it now sits south of the yard rather than east of the old Grand
 *  Farm. The heading is a hint, not an identifier -- the rail is sorted by
 *  `zonesByDistance` and labelled with the district's own name, which is what
 *  actually tells two of the same heading apart. */
