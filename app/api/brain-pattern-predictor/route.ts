...
import { z } from "zod";
import {
  openBrainPatternPredictor,
  readBrainPatternPredictor,
...
export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-pattern-predictor:read", 120, 60 * 1000);
  if (limited) return limited;

  const token = readOrCreateSessionToken(request);
  try {
    return withRequestSessionCookie(request, NextResponse.json(await readBrainPatternPredictor(token)), token);
  } catch (error) {
...
export async function POST(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-pattern-predictor:start", 30, 60 * 1000);
  if (limited) return limited;
  ...
  try {
    ...
    const result = await openBrainPatternPredictor(token, parsed.data.wager);
