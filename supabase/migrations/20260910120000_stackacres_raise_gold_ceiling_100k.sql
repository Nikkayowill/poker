-- Raise the StackAcres daily Gold ceiling: 50,000 -> 100,000 per player per UTC day.
--
-- Kayo's call, 2026-09-10. Same reasoning as the 15k->50k raise: cosmetics run up
-- to 350,000 Gold, so a bigger daily yield gets a player there in fewer sessions.
-- The ceiling is still FLAT, still PER PLAYER PER UTC DAY, and the database is
-- still the authority. A TypeScript bug can lower this but never raise it.
--
...
as $$
declare
  -- 5000 -> 15000 -> 50000 -> 100000. Updated 2026-09-10.
  hard_ceiling constant integer := 100000;
...
comment on function public.reserve_homestead_exchange(uuid, date, integer, integer) is
  'Atomically reserves Gold against a player''s flat daily StackAcres ceiling (100,000/day since 20260910120000). Returns the day''s new total, or NULL when the ceiling refuses -- callers treat NULL as a lost race and never pay on it. Carries its own hard ceiling; the p_ceiling argument can only tighten it.';
