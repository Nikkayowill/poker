...
import { MIN_ANTE_UP_WAGER } from "@/lib/arcade/ante-up-memory";
...
/** Ante Up: Memory Match's lobby. Same shape as app/api/ante-up/route.ts (Sudoku) minus the difficulty pick. */
...
export async function GET(request: NextRequest) {
  ...
  try {
    return withRequestSessionCookie(request, NextResponse.json(await readAnteUpMemory(token)), token);
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await openAnteUpMemory(token, parsed.data.wager);
