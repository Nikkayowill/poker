...
export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-quick-math:read", 120, 60 * 1000);
  if (limited) return limited;

  const token = readOrCreateSessionToken(request);
  try {
    return withRequestSessionCookie(request, NextResponse.json(await readBrainQuickMath(token)), token);
  } catch (error) {
...
export async function POST(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-quick-math:start", 30, 60 * 1000);
  if (limited) return limited;
  ...
  try {
    ...
    const result = await openBrainQuickMath(token, parsed.data.wager);
