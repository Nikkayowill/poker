 ...
 * one, since a rotation that never rotates is machinery a reader has to
 * disprove. Recover it from git history if the house ever hires again.
 *
 * A redraw should never need a per-dealer number here. A plate arrives at its
 * own crop with its own idea of where the middle is; `scripts/prepare-dealer.py`
 * absorbs that, normalising it onto a box with the crown at the top edge, the
 * hands at the bottom edge and the head centred. The app holds a single
 * placement, `DEALER_SLOT` below, and a new drawing is a file dropped in
 * `art/dealers/` plus a run of that script. If you find yourself adding an
 * offset here, the plate is framed wrong; fix it in the source.
 *
 * The slot is solved from the camera rather than fixed in pixels: its size is
 * a multiple of the projected gap between the two chairs flanking the dealer,
 * which the scene reports every frame, so the dealer grows and shrinks with
 * the table like the seats and the board do, across viewport sizes,
 * headcounts, and device rotation.
 */
 ...
 * keep the dealer behind the table rather than leaning over it. At far
 * centre the camera looks slightly down at someone a good half-metre further
 * away than the near rail, so a flat bitmap drawn at full size with its
 * hands on the cloth reads as being in front of the table. Drawing it a
 * little short of the full gap between the chairs supplies the distance cue
 * the artwork can't give on its own; everything else here gets its size from
 * the perspective divide, and this is a fixed bitmap.
 */
export const DEALER_SLOT = {
   ...
   *
   * Positive lifts the dealer. It's small, and anchored to hair rather than
   * skull: `fitCamera` reserves its top margin against head points and hair
   * is what occupies that margin, so anchoring the skull clips a ponytail
   * off the top of the frame and drops the hands onto the rail instead of
   * the cloth.
   */
  crown: 0.02,
} as const;
 ...
 * so everything below is re-solved whenever the camera is; nothing here is a
 * fixed size.
 */
