...
import { z } from "zod";
import {
  openBrainSequenceRecall,
  readBrainSequenceRecall,
...
/** Sequence Recall's lobby. Same shape as app/api/ante-up-memory/route.ts. */
const startSchema = z.object({ wager: z.number().int().min(0) });

export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-sequence-recall:read", 120, 60 * 1000);
  if (limited) return limited;

  const token = readOrCreateSessionToken(request);
  try {
    return withRequestSessionCookie(request, NextResponse.json(await readBrainSequenceRecall(token)), token);
  } catch (error) {
...
export async function POST(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-sequence-recall:start", 30, 60 * 1000);
  if (limited) return limited;
  ...
  try {
    ...
    const result = await openBrainSequenceRecall(token, parsed.data.wager);
