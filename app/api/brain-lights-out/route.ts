...
import { MIN_ANTE_UP_WAGER } from "@/lib/arcade/brain-lights-out";
...
export async function GET(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-lights-out:read", 120, 60 * 1000);
  if (limited) return limited;

  const token = readOrCreateSessionToken(request);
  try {
    return withRequestSessionCookie(request, NextResponse.json(await readBrainLightsOut(token)), token);
  } catch (error) {
...
export async function POST(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-lights-out:start", 30, 60 * 1000);
  if (limited) return limited;
  ...
  try {
    ...
    const result = await openBrainLightsOut(token, parsed.data.wager);
