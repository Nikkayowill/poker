...
import { readWordlePuzzle, startWordlePuzzle, toWordleErrorResponse } from "@/lib/server/wordle-service";
 ...
 * back is toWordleSnapshot()'s redacted view -- the tiles it has already
...
export async function GET(request: NextRequest) {
  ...
  try {
    return withRequestSessionCookie(request, NextResponse.json(await readWordlePuzzle(token)), token);
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  const limited = enforceRateLimit(request, "arcade:wordle:start", 60, 60 * 1000);
  ...
  try {
    ...
    return withRequestSessionCookie(request, NextResponse.json(await startWordlePuzzle(token)), token);
  } catch (error) {
    return withRequestSessionCookie(request, toWordleErrorResponse(error), token);
  }
}
