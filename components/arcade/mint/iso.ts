import { MINT_GRID_PLOTS } from "@/lib/mint/nodes";
 ...
 *
 * Deliberately free of any Phaser import: mint-treasury.tsx reads this for
 * the overlay, and Phaser must only ever enter the browser through
 * mint-canvas.tsx's dynamic import.
 */

export const MINT_STAGE_W = 720;
/**
 ...
 */
export const MINT_STAGE_H = 470;

/** Half-extents of a tile diamond, in stage pixels. */
export const MINT_TILE_HALF_W = 66;
export const MINT_TILE_HALF_H = 41;
 ...
 */
export const MINT_TAP_HALF_W = STEP_X;
export const MINT_TAP_HALF_H = STEP_Y;

/** Where the diamond grid's row-0/col-0 corner tile centers. */
const ORIGIN_X = MINT_STAGE_W / 2;
const ORIGIN_Y = 108;

/** Center of plot `plotIndex` (1-based, row-major 4x4) in stage pixels. */
export function mintTileCenter(plotIndex: number): { x: number; y: number } {
...
export function mintPaintOrder(): number[] {
  return Array.from({ length: MINT_GRID_PLOTS }, (_, i) => i + 1).sort(
    (a, b) => mintTileCenter(a).y - mintTileCenter(b).y,
  );
}
