...
describe("the daily ceiling", () => {
  it("is a flat number, not a function of anything", () => {
    ...
    // the change that turns the Homestead back into a scaling faucet, and it
    // should have to delete this test to happen.
    expect(typeof HOMESTEAD_GOLD_CEILING).toBe("number");
    expect(HOMESTEAD_GOLD_CEILING).toBe(5_000);
  });
  ...
  it("sits alongside the faucets that already exist, not above them", () => {
    ...
    // ~7,500/day the uncapped Gold Homestead paid.
    expect(HOMESTEAD_GOLD_CEILING).toBeGreaterThan(3_000);
    expect(HOMESTEAD_GOLD_CEILING).toBeLessThan(7_500);
  });

  it("bounds a request at exactly one day's worth of Bushels", () => {
    expect(goldForBushels(HOMESTEAD_MAX_EXCHANGE_BUSHELS)).toBeGreaterThanOrEqual(
      HOMESTEAD_GOLD_CEILING,
    );
    expect(goldForBushels(HOMESTEAD_MAX_EXCHANGE_BUSHELS - 1)).toBeLessThan(
      HOMESTEAD_GOLD_CEILING,
    );
  });
});
...
describe("the rate", () => {
  it("pays the same for the first Bushel as for the last", () => {
    ...
    expect(goldForBushels(1)).toBe(HOMESTEAD_GOLD_PER_BUSHEL);
    expect(goldForBushels(1_000)).toBe(1_000 * HOMESTEAD_GOLD_PER_BUSHEL);
  });

  it("rounds an allowance DOWN, so a Bushel never buys nothing", () => {
    expect(bushelsWithinAllowance(HOMESTEAD_GOLD_PER_BUSHEL * 3)).toBe(3);
    expect(bushelsWithinAllowance(HOMESTEAD_GOLD_PER_BUSHEL * 3 - 1)).toBe(2);
...
describe("the day boundary", () => {
  it("is UTC, matching the daily grant rather than the device", () => {
    expect(homesteadExchangeDay(new Date("2026-09-01T00:00:00.000Z"))).toBe("2026-09-01");
    expect(homesteadExchangeDay(new Date("2026-09-01T23:59:59.999Z"))).toBe("2026-09-01");
    expect(homesteadExchangeDay(new Date("2026-09-02T00:00:00.000Z"))).toBe("2026-09-02");
  });
...
describe("what the client is told", () => {
  it("reports what is left, and what that is worth in Bushels", () => {
    ...
    expect(state.ceiling).toBe(HOMESTEAD_GOLD_CEILING);
    expect(state.usedToday).toBe(1_000);
    expect(state.remaining).toBe(HOMESTEAD_GOLD_CEILING - 1_000);
    expect(state.maxBushels).toBe(bushelsWithinAllowance(HOMESTEAD_GOLD_CEILING - 1_000));
  ...
  it("never reports a negative allowance, whatever the stored total says", () => {
    ...
    const state = exchangeState(HOMESTEAD_GOLD_CEILING + 900, new Date());
