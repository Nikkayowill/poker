 ...
 *
 * No loading placeholder. The DOM table is complete on its own (the felt and
 * rail keep painting until `onReady` says the room exists), so the scene
 * arriving a moment later is a table that gets lit, not a table with a hole
 * in it.
 */
 ...
 * the 3D room, since it is Canvas 2D and shares the chip layer already in
 * the table chunk, but the table's own table-anchors geometry and room
 * painter are dead weight for everyone on the classic renderer.
 */
 ...
 * unmodified until now): it reads big on the cloth like a real dealt hand
...
export function PokerTable({
  ...
  /* Render gate: nothing paints until the renderer choice is genuinely
     ...
     deferred set in use-stored-preference.ts), and DEFAULT_TABLE_RENDERER is
     `webgl_3d`, so without this a player who chose the classic table watched
     the 3D room mount, acquire a GL context, paint, and get torn down again
     on the very next commit. A blank hold is cheaper than a discarded room
     and reads as a load rather than as a glitch.
