...
export async function openAnteUpNonogram(
  ...
  const wagerCorrelationId = `ante_up_nonogram_wager:${randomUUID()}`;
  const debited =
    wagerInput > 0
      ? await spendStakeLedgered(profile, wagerInput, wagerCorrelationId, "ante_up_nonogram_wager")
      : profile;
  if (!debited) {
    throw new AnteUpNonogramRequestError(
  ...
  try {
    ...
    if (wagerInput > 0) {
      await creditGoldByProfileLedgered(
        ...
        wagerInput,
        wagerCorrelationId,
        "ante_up_nonogram_wager_refund",
      ).catch((refundError) => {
        console.error("ante_up_nonogram.open_refund_failed", {
          profileId: profile.id,
          wager: wagerInput,
          error: refundError,
        });
  ...
  if (wagerInput > 0) {
    await confirmGoldDebitLedgered(wagerCorrelationId).catch((confirmError) => {
      console.error("ante_up_nonogram.open_confirm_failed", {
        profileId: profile.id,
        wagerCorrelationId,
        error: confirmError,
      });
