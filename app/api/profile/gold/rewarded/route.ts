 ...
 *
 * Parse, authorise, respond. Every rule lives in
 * lib/server/rewarded-ad-service.ts, which is where `npm test` can reach it
 * (vitest.config.ts collects lib/ and app/ only, and money-moving logic
 * buried in a handler is awkward to test). Same division the arcade routes
 * use.
 *
 * The limit is tighter than the other Gold routes' 10/minute: a legitimate
 * player cannot start more than two ads a minute since each one takes thirty
 * seconds, so anything above that is a script, and the grant table's
