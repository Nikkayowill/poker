...
import { z } from "zod";
import {
  answerBrainTriviaBlitz,
  resignBrainTriviaBlitz,
...
export async function POST(request: NextRequest) {
  const limited = await enforceRateLimit(request, "brain-trivia-blitz:act", 600, 60 * 1000);
  ...
  try {
    ...
    const result = parsed.data.action === "resign"
      ? await resignBrainTriviaBlitz(token)
      : await answerBrainTriviaBlitz(token, { version: parsed.data.version, given: parsed.data.given });
