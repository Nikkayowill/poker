 ...
 * wholesale with whatever the API returns, since every action is a request
 * and the response is the new truth.
 *
 * Nothing is dealt or decided here. The client can't see the undealt deck
 * (the snapshot has no `deck` field) or the dealer's hole card until the
 * dealer's turn, so it can't report a result either, which is what makes it
 * safe for this table to settle real Gold. Every button below is a hint
 * about what the server will accept, never a decision; `snapshot.legal`
 * comes from the same engine call the route re-runs before it moves anything.
 */

/*
 * DealerStage is a plain import, not lazy-loaded. It's four positioned 2D
 * layers with no renderer or GPU context, so there's no bundle-size cost to
 * defer.
 */
...
export function BlackjackTable() {
  ...
  /**
   * Practice hand: a $0 deal with nothing at risk. A separate boolean rather
   * than a rung on the tier ladder, since STAKES_TIERS also drives the
   * real-money poker lobby's buy-in flow and a free entry there would leak a
   * $0 buy-in into table selection. Picking a real tier below turns this off.
   */
  ...
  /**
   * The tip jar's cadence lives in client state, not the server.
   *
   * How often the pair ask is presentation and costs nothing if a refresh
   * forgets it; how much Gold actually left the wallet is not, and that lives
   ...
   * from a click handler, and every path that can produce a settled hand
   * (an action, a resume, a 409 carrying the true state) goes through `round`,
   ...
   * swallowed, since this table moves real Gold and "nothing happened" is not
  ...
  /**
   * Tipping the pair. A pure sink: the Gold leaves and nothing comes back but
   * XP and a thank-you, which is exactly what a tip is.
   *
  ...
  return (
    <main className="bj-shell">
      ...
      <section className="bj-felt" aria-live="polite" aria-busy={busy}>
        ...
        {/*
         * No avatar on this hand. Adding one back would draw Loki and Finn
         * twice: once in the stage above, once again here in a different art
         * style, both labelled "Loki & Finn". The pair are in the room; this
        ...
        <Hand
          ...
          // PlayingCard's `back` prop exists to fix.
