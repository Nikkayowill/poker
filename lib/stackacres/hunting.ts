/**
 * What lives in the Oak's brush, what taking one gives, and which weapon is
 * in hand.
 *
 * Pure, and split from ./hunt-scope.ts the same way ./fishing.ts is split
 ...
 * client's -- the scope only decides that the player earned one.
 *
...
export interface QuarryDef {
  ...
  /** Meat and pelts one of these yields. Both always at least 1, so no
   *  completed stalk ever ends in an empty shelf. */
  readonly meat: number;
  readonly pelt: number;
}

/**
 * Priced through ./machine-items.ts's own meat/pelt sell prices rather than
 * here, so one stalk is worth roughly what one cast is: a Rabbit lands
 * around a Bluegill, a Deer around a Trout, a Boar a little under a Catfish.
 * The ladder is deliberately flatter than fishing's -- a Boar is 13% of
 * stalks against a Catfish's 10%, and the stalk itself is the harder game.
 */
...
export const HUNTING_WEAPON_DEFS: Readonly<Record<HuntingWeapon, HuntingWeaponDef>> = {
  bow: {
    label: "Hunting bow",
    hint: "Track something in the brush and hold it steady.",
    icon: "ico-bow",
  },
  rifle: {
    label: "Rifle",
    hint: "Steadier than the bow, and it settles on a mark far quicker.",
    icon: "ico-rifle",
  },
};

/**
 * The rifle's gate. DERIVED, NEVER STORED, the same rule ./shop-locks.ts
 * sets for Ray's shelf and ./story/unlocks.ts for the travelers: this is
 * `levelUnlock(4)`, which is the very same milestone count the Mine already
 * opens on, so a player reading "Requires: three milestones" in two places
 * is being told the same thing by the same code.
 *
 * A stored `owns_rifle` column was the alternative and was not taken: it
...
/** Whether this farm has earned the rifle yet. */
export function rifleUnlocked(progress: StackAcresShopProgress): boolean {
  return storyLevel(progress) >= RIFLE_LEVEL;
}

/** The best weapon this farm may carry: the rifle once it is earned, the bow
 *  before that. There is never a weaponless state -- the bow is the floor,
 *  the way `hand` is the belt's resting slot. */
export function bestWeapon(progress: StackAcresShopProgress): HuntingWeapon {
  return rifleUnlocked(progress) ? "rifle" : "bow";
}

/** What the locked rifle slot says it is waiting for. */
