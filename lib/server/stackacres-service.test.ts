 ...
 * exactly once at stocking, the snapshotted yield is credited exactly once at
...
const SPROUT = STACKACRES_CATALOGUE.carrot;
...
describe("stocking", () => {
  ...
  it("marks an animal fed on arrival and leaves a crop with no feed clock", async () => {
    ...
    const fieldView = await stockStackAcres(token, { stock: "carrot" }, T0);

    expect(unitOf(henView, "hen").hungryAt).not.toBeNull();
    const carrot = unitOf(fieldView, "carrot");
    expect(carrot.hungryAt).toBeNull();
  });
  ...
  it("never sells a tier-1 crop outright, and takes nothing for trying", async () => {
    // Tier 1 is the hand-worked tier -- seed only, at any purse size.
    const { token } = await funded();
    const before = await balance(token);

    await expect(buyStackAcresStock(token, { stock: "carrot" }, T0)).rejects.toBeInstanceOf(
      StackAcresRequestError,
    );
  ...
  it("stocks a crop by spending a seed off the shelf, never Gold", async () => {
    ...
    expect(await balance(token)).toBe(before);
    // It goes in as seed waiting for its first water.
    expect(unitOf(view, "carrot").state).toBe("dry");
    expect(unitOf(view, "carrot").progress).toBe(0);
...
describe("buyStackAcresSeed — Ray's shelf", () => {
  ...
  it("refuses when Gold is short and shelves nothing", async () => {
    ...
    await adjustStackAcresSeedStock(id, "carrot", -1000);
    // One bag stopped being enough to overdraw 10 Gold once tier 1's seed
    // dropped to 1 Gold, so order the most one purchase allows.
    const quantity = STACKACRES_SEED_BAGS_PER_PURCHASE;
    expect(STACKACRES_CATALOGUE.carrot.seedCost * quantity).toBeGreaterThan(10);
...
describe("the watering can", () => {
  ...
  it("spends nothing on ground that is still wet", async () => {
    const { token, id } = await funded();
    const view = await stockStackAcres(token, { stock: "carrot" }, T0);
    const unitId = unitOf(view, "carrot").id;
    await waterStackAcres(token, unitId, T0);

    await waterStackAcres(token, unitId, new Date(T0.getTime() + 1000));
    expect(await readStackAcresWater(id)).toBe(WATER_CAPACITY - 1);
  });
...
describe("thirst", () => {
  // Corn, not the carrot this block used to sow: tier 1 ripens in 15s now,
  // long before its 8m thirst window, so it can no longer go dry mid-cycle.
  // Corn's cycle outlasts two of its thirst windows, so a test that needs it
  // to reach the finish line waters it through each drought.
  const CROP = STACKACRES_CATALOGUE.corn;
  const THIRST = CROP.thirstMs ?? 0;
  /** Corn sown and given its first water at T0, so its clock starts there
   *  and its first drought is at T0 + THIRST. */
  const stackacresSprout = async (token: string) => {
    const sown = await stockStackAcres(token, { stock: "corn" }, T0);
    return waterStackAcres(token, unitOf(sown, "corn").id, T0);
  };
  const dryAt = new Date(T0.getTime() + THIRST + 1000);
  /** Waters at every drought after `lastWatered` until the unit's finish
   *  line, each exactly as the ground dries so none of them adds time.
   *  Returns that finish line. */
  const waterUntilRipe = async (token: string, id: string, unitId: string, lastWatered: Date) => {
    const readyAt = Date.parse((await getStackAcresUnit(id, unitId))?.readyAt ?? "");
    for (let drink = lastWatered.getTime() + THIRST; drink < readyAt; drink += THIRST) {
      await waterStackAcres(token, unitId, new Date(drink));
    }
    return readyAt;
  };
  ...
  it("freezes a field past its watering window instead of letting it finish", async () => {
    ...
    const unitId = unitOf(view, "corn").id;

    // Well past readiness, but the soil went dry long before that.
    const wayLater = new Date(T0.getTime() + CROP.durationMs + 60_000);
    const later = await readStackAcres(token, wayLater);
    expect(unitOf(later, "corn").state).toBe("dry");
    expect(unitOf(later, "corn").isWatered).toBe(false);
  ...
  it("lets the field finish once watered, on the pushed-out clock", async () => {
    ...
    expect(unitOf(resumed, "corn").state).toBe("working");
    expect(unitOf(resumed, "corn").isWatered).toBe(true);

    // Ready at its own pushed-out clock -- the time the field stood dry was
    // ADDED to the cycle, not credited to it. Each later drought is watered
    // right as it lands, so none of them moves that clock again.
    const readyAt = await waterUntilRipe(token, id, unitId, wateredAt);
    expect(readyAt).toBe(Date.parse(unitOf(resumed, "corn").readyAt));
    const done = await readStackAcres(token, new Date(readyAt));
    expect(unitOf(done, "corn").state).toBe("ready");
  });
  ...
  it("still harvests a crop that ripened before its ground dried", async () => {
    ...
    // Water through each drought, so the crop finishes at its own clock and
    // the ground only gives out after that last drink runs dry.
    const readyAt = await waterUntilRipe(token, id, unitId, T0);
    ...
    expect(unitOf(late, "corn").state).toBe("ready");

    // And it actually yields -- a drought after the harvest was made takes
    // nothing away from it.
    const before = await balance(token);
    const paid = await collectOne(token, unitId, muchLater);
    expect(paid.harvest.tally).toEqual([
      { item: STACKACRES_YIELDS.corn.item, quantity: STACKACRES_YIELDS.corn.quantity },
    ]);
    expect(paid.inventory.corn).toBe(STACKACRES_YIELDS.corn.quantity);
    expect(await balance(token)).toBe(before);
  });
  ...
  it("restarts a bought crop as seed, and its first water starts a fresh cycle", async () => {
    ...
    // First water at T0, then through every drought so it actually reaches
    // the finish line.
    await waterStackAcres(token, unitId, T0);
    await waterUntilRipe(token, id, unitId, T0);
    ...
    // Back in as seed waiting for water...
    const restarted = await getStackAcresUnit(id, unitId);
    expect(restarted?.lastWateredAt).toBeNull();
    const fresh = unitOf(await readStackAcres(token, collectedAt), "corn");
    expect(fresh.state).toBe("dry");
    expect(fresh.progress).toBe(0);

    // ...and watering it however much later starts a full cycle from then,
    // never charging the time it sat ripe or sat as seed.
    const wateredAt = new Date(collectedAt.getTime() + 2 * 60 * 60 * 1000);
    await waterStackAcres(token, unitId, wateredAt);
    const growing = await getStackAcresUnit(id, unitId);
    expect(growing?.startedAt).toBe(wateredAt.toISOString());
    expect(Date.parse(growing?.readyAt ?? "")).toBe(wateredAt.getTime() + CROP.durationMs);
  });

  it("sows seed that waits for water, then grows on a cycle started by its first water", async () => {
    const { token, id } = await funded();
    const view = await stockStackAcres(token, { stock: "corn" }, T0);
    const unitId = unitOf(view, "corn").id;

    // An hour on, still seed: nothing grew while it sat unwatered.
    const anHourOn = new Date(T0.getTime() + 60 * 60 * 1000);
    const waiting = unitOf(await readStackAcres(token, anHourOn), "corn");
    expect(waiting.state).toBe("dry");
    expect(waiting.progress).toBe(0);

    const watered = unitOf(await waterStackAcres(token, unitId, anHourOn), "corn");
    expect(watered.state).toBe("working");
    expect(watered.progress).toBe(0);
    const row = await getStackAcresUnit(id, unitId);
    expect(row?.startedAt).toBe(anHourOn.toISOString());
    expect(Date.parse(row?.readyAt ?? "")).toBe(anHourOn.getTime() + CROP.durationMs);
  });
  ...
  it("never lets neglect cost produce: the snapshotted yield is untouched by watering", async () => {
    ...
    expect((await getStackAcresUnit(id, unitId))?.yieldQuantity).toBe(
      STACKACRES_YIELDS.corn.quantity,
    );
  });
...
describe("harvesting", () => {
  it("credits the snapshotted yield to inventory, exactly once, and moves no Gold", async () => {
    ...
    // removes it) and credits nothing more.
    await expect(collectOne(token, unitId, HEN_READY)).rejects.toBeInstanceOf(
      StackAcresRequestError,
    );
    expect((await readStackAcres(token, HEN_READY)).inventory.eggs).toBe(HEN_YIELD.quantity);
    expect(await balance(token)).toBe(before);
  });
  ...
  it("credits a cow's milk to inventory, and moves no Gold", async () => {
    const { token, id } = await funded();
    const cow = await createStackAcresUnit(id, {
      stock: "cattle",
      stake: CATTLE.seedCost,
      yieldQuantity: STACKACRES_YIELDS.cattle.quantity,
      startedAt: new Date(T0.getTime() - CATTLE.durationMs),
      readyAt: T0,
      lastFedAt: T0,
      lastWateredAt: null,
      permanent: false,
    ...
    const before = await balance(token);
    const result = await collectOne(token, cow.id, T0);
  ...
  it("yields what the ROW says, not what the catalogue says today", async () => {
    ...
    expect(result.harvest.gross).toBe(itemSellPrice(HEN_YIELD.item) * retuned);
    expect(result.inventory.eggs).toBe(retuned);
    expect(await balance(token)).toBe(before);
  });
...
describe("the equipment ladder", () => {
  ...
  it("adds bonus produce to a critical harvest, not Gold", async () => {
    ...
    // Golden Spade: a bonus of 1 doubles the line, in kind.
    expect(result.harvest.critBonus).toEqual([{ item: "eggs", quantity: HEN_YIELD.quantity }]);
    expect(result.inventory.eggs).toBe(HEN_YIELD.quantity * 2);
    expect(await balance(token)).toBe(before);
  });
...
describe("the currency wall", () => {
  ...
  it("exposes exactly three actions that pay Gold out, all ceiling-gated", () => {
    ...
    // cannot be lost in a rename: `sell`, `fulfill-contract` and
    // `collect-vat` are the only three actions in this route file that pay a
    // player Gold.
    ...
    // Flour/Cloth, never Gold). `collect` moves no Gold either: it credits
...
describe("idempotency keys", () => {
  const run = <T,>(token: string, key: string | null, action: string, fn: () => Promise<T>) =>
    // T0, not the real clock: every action now assesses the day's Land
    // Maintenance first, and funded() only pre-pays T0's day.
  ...
  it("sows once when the same key arrives twice", async () => {
    ...
    await run(token, key, "stock", () => stockStackAcres(token, { stock: "carrot" }, T0));
  ...
  it("hands a replayed harvest the same produce line, not a refusal", async () => {
    ...
    const creditedOnce = (await readStackAcres(token, HEN_READY)).inventory.eggs;
    expect(creditedOnce).toBe(HEN_YIELD.quantity);
  ...
  it("frees the key when the action refused, so the retry is a real attempt", async () => {
    ...
    expect(retried.units.filter((u) => u.stock === "carrot")).toHaveLength(1);
  });
  ...
  it("runs unguarded when no key is sent, so an older client still works", async () => {
    ...
    await run(token, null, "stock", () => stockStackAcres(token, { stock: "carrot" }, T0));
  ...
  it("keeps one player's key clear of another's", async () => {
    ...
    await run(a.token, key, "stock", () => stockStackAcres(a.token, { stock: "carrot" }, T0));
    const other = await run(b.token, key, "stock", () =>
      stockStackAcres(b.token, { stock: "carrot" }, T0),
    );
...
describe("wheat and machines", () => {
  ...
  it("does not start a mill short of its full input batch", async () => {
    ...
    expect(WHEAT_YIELD_QUANTITY).toBeLessThan(RECIPE_CATALOGUE.flour.inputs[0].quantity * 2);
...
describe("recipes", () => {
  ...
  describe("harvesting into the shared inventory", () => {
    it("takes a cow's milk straight to the shelf and pays no Gold for it", async () => {
      const { token, id } = await funded();
      const unit = await readyAnimal(id, "cattle");
      const before = await balance(token);

      const result = await harvestStackAcres(token, { unitIds: [unit.id] }, T0);

      expect(result.harvest.tally).toEqual([{ item: "milk", quantity: STACKACRES_YIELDS.cattle.quantity }]);
      expect(result.inventory.milk).toBe(STACKACRES_YIELDS.cattle.quantity);
      expect(await balance(token)).toBe(before);
    });

    it("takes a sheep's wool the same way", async () => {
      const { token, id } = await funded();
      const unit = await readyAnimal(id, "pig");
      const result = await harvestStackAcres(token, { unitIds: [unit.id] }, T0);
      expect(result.inventory.wool).toBe(STACKACRES_YIELDS.pig.quantity);
    });
  });
  ...
  describe("processRecipe", () => {
    it("converts in one step for an instant recipe, and moves no Gold", async () => {
      ...
      const view = await readStackAcres(token, T0);
      expect(view.inventory.milk).toBe(7 - RECIPE_CATALOGUE.cheese.inputs[0].quantity);
    ...
    it("refuses short of a full batch and spends nothing", async () => {
      ...
      await adjustStackAcresInventory(id, "wool", RECIPE_CATALOGUE.cloth.inputs[0].quantity - 1);
      ...
      const view = await readStackAcres(token, T0);
      expect(view.inventory.wool).toBe(RECIPE_CATALOGUE.cloth.inputs[0].quantity - 1);
    ...
    it("enqueues a queued recipe onto the machine, snapshotting what it will pay", async () => {
      ...
      expect(running.inventory.wheat).toBe(5 - RECIPE_CATALOGUE.flour.inputs[0].quantity);
    ...
    it("refuses a second batch while the machine is still running the first", async () => {
      ...
      expect(view.inventory.wheat).toBe(99 - RECIPE_CATALOGUE.flour.inputs[0].quantity);
    });
    ...
    it("bakes a Cake from 2 Eggs, 1 Milk and 1 Flour in one step, and moves no Gold", async () => {
      const { token, id } = await funded();
      await placeStackAcresMachine(token, "dairy", T0);
      await adjustStackAcresInventory(id, "eggs", 2);
      await adjustStackAcresInventory(id, "milk", 1);
      await adjustStackAcresInventory(id, "flour", 1);
      const before = await balance(token);

      const result = await processRecipe(id, "cake", T0);

      expect(result.produced).toEqual({ item: "cake", quantity: 1 });
      expect(result.readyAt).toBeNull();
      const view = await readStackAcres(token, T0);
      expect(view.inventory).toMatchObject({ eggs: 0, milk: 0, flour: 0, cake: 1 });
      expect(view.machines[0].status).toBe("idle");
      expect(await balance(token)).toBe(before);
    });

    it("refuses a Cake short of any one input and changes nothing", async () => {
      const { token, id } = await funded();
      await placeStackAcresMachine(token, "dairy", T0);
      await adjustStackAcresInventory(id, "eggs", 2);
      await adjustStackAcresInventory(id, "milk", 1);
      ...
      // No flour at all.
      await expect(processRecipe(id, "cake", T0)).rejects.toBeInstanceOf(StackAcresRequestError);
      let view = await readStackAcres(token, T0);
      expect(view.inventory).toMatchObject({ eggs: 2, milk: 1 });
      expect(view.inventory.cake ?? 0).toBe(0);

      // Flour on hand, one egg short.
      await adjustStackAcresInventory(id, "flour", 1);
      await adjustStackAcresInventory(id, "eggs", -1);
      await expect(processRecipe(id, "cake", T0)).rejects.toBeInstanceOf(StackAcresRequestError);
      view = await readStackAcres(token, T0);
      expect(view.inventory).toMatchObject({ eggs: 1, milk: 1, flour: 1 });
      expect(view.inventory.cake ?? 0).toBe(0);
    });
  });
...
describe("sellStackAcresItem", () => {
  ...
  it("takes the goods off the shelf, then credits their Gold once", async () => {
    const { token, id } = await funded();
    await adjustStackAcresInventory(id, "eggs", 10);
    const before = await balance(token);
  ...
  it("refuses more than is held, and moves no Gold and no goods", async () => {
    const { token, id } = await funded();
    await adjustStackAcresInventory(id, "eggs", 2);
    const before = await balance(token);
