 ...
 *
 * Adsterra's invoke script is written for a 1990s page: it reads window
 * .atOptions and then `document.write`s the ad's markup at the point in the
 * document where the <script> tag sits. That works exactly once -- during the
 * initial parse. Appended afterwards, on a document that has already closed,
 * `document.write` implicitly calls `document.open()`, which BLANKS THE ENTIRE
 * PAGE. In a single-page React app the parse is over before any route mounts,
 * so any version of this that injects the loader into the main document is one
 * ad-network deploy away from wiping the table mid-hand.
 *
 * So the unit gets its own document. `srcDoc` hands the iframe markup it then
 * parses from scratch, which is the one situation `document.write` is defined
 * for -- and whatever it writes lands in a document that is not ours.
 *
 * WHY THE SANDBOX IS THIS NARROW
 *
 * `allow-scripts` and nothing else. In particular:
 *
 *   - no `allow-same-origin`, so the frame gets an opaque origin and cannot
 *     reach into this document, read our cookies, or touch localStorage. (The
 *     pair `allow-scripts allow-same-origin` on a same-origin frame is not a
 *     sandbox at all -- the frame can simply remove its own sandbox attribute
 *     from the parent DOM.)
 *   - no `allow-popups` and no `allow-top-navigation`, which is the actual
 *     point. CLAUDE.md records that this vendor is known for popunders and
 *     redirect creative, and CSP governs only where a resource loads *from*,
 *     never what it does once loaded. These two flags are what govern that.
 *
 * A unit that needs those permissions to render does not get them; it gets to
 * not render. The reward in components/rewards/rewarded-ad-modal.tsx is
 * deliberately not conditional on this frame doing anything, so a blocked or
 * empty ad costs the player nothing.
 *
 * WHY THE BOX IS RESERVED
 *
 * The wrapper is sized from the props before the frame exists and never
 * changes size afterwards. An ad slot that grows on load is a layout shift,
 * and this app has a header whose height is a computed contract
 * (`calc(var(--game-header-h) + var(--safe-top))`, asserted at 127px under an
 * iPhone 14's insets by tests/e2e/safe-area.spec.ts). Nothing here may push on
 * that, so the slot is a fixed box from first paint and the frame fills it.
 */
 ...
 * the answer never changes again once it is true.
 *
