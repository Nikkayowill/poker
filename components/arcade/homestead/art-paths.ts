...
// at the top of homestead-art.ts).
import type * as Phaser from "phaser";
import { distanceToPath, pathBounds, type PathSpec } from "@/lib/homestead/paths";
import { powerOfTwoCeil, seededRandom, type WorldPoint } from "@/lib/homestead/world";
 ...
 * cream stones a little way off ONE side. The polyline from lib/homestead/
...
export function bakePathTexture(scene: Phaser.Scene, spec: PathSpec, under: readonly PathSpec[]): PathBake | null {
  ...
  if (texW > 2048 || texH > 2048) {
    console.warn(`homestead: path ${spec.key} would bake at ${texW}x${texH}; skipped`);
    return null;
  }
