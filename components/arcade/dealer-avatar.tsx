 ...
 * beside the dealer's hand; that call was removed once the pair moved into
 * the room above their own cards, since a second drawing of them forty pixels
 * below was the duplication that made the old stage read as a television
 * rather than a table.
 *
 * It's still built to survive 34px, and that constraint shapes every decision
 * below (see the note on ears). Judge any change here against a real small
 ...
 *
 * Framed as a face crop, not two figures, which is the lesson the previous
 * dealer cost to learn. Every player avatar beside it is avatarFace(), a head
 * filling the disc, and the first version of that dealer drew a whole
 * croupier inside the circle: at the size it actually renders, the head was
 * eleven pixels and the hat a bar across it. Two heads fill this frame edge
 * to edge, and every dog carries five marks at most.
 *
 * The uniform is a shirt collar and a black bow tie; it used to be a green
 * croupier's visor and a gold one. Neither visor nor gold exists in the real
 * portraits (public/dealer/), and the mismatch survived a long time because
 * this drawing is Blackjack's placeholder: it stops rendering there the
 * moment real art lands, so the drift was invisible on the one page anyone
 * was looking at while staying live on the five other arcade games that still
 * draw it. If this file and the photographs disagree again, that's the bug.
 *
 ...
 * against the owner's own reference: they're two doodles with hanging, curly
    ...
    >
      <svg viewBox="0 0 64 64" aria-hidden="true" focusable="false">
        ...
        <g>
          {/* Drop ears.
           *
           * Low and long, hanging past the jaw: the single most load-bearing
           * decision in the drawing. A first cut put both dogs' ears as round
           * lobes level with the eyes and outside the skull, which reads as a
           * bear. An ear that starts above the eye and hangs to below the
           * muzzle is what says "dog", and on a doodle it's most of the
           * silhouette. */}
          ...
          {/* The rim: light catching the top of a curly coat. Low opacity so
              it reads as a highlight rather than a drawn outline; without it
              a black dog on this dark disc has no silhouette at all. */}
           ...
           * read as a light band, a dark band, then a lighter skull -- a hat,
           * a hatband and a brim, with the dark ellipse's lower arc reading
           * as the brim's edge. With a visor over it that was hair; without
           * one it's a bowler hat on a dog.
           *
           * Shadow can't fix it, worth remembering: Finn's coat is a very
           * dark warm grey chosen to sit just off black, so there's no darker
           * value left to shade with, and any dark mass laid on that skull
           * stops being shading and becomes an object. Light is the only axis
           * he has, the same trick the ear rim above already uses for the
           * same reason. Lobes follow the skull's curve so there's no
           * horizontal edge anywhere for an eye to read as a brim.
           *
           * Loki's equivalent below is a dark fringe and is fine: apricot has
           * somewhere darker to go, so his reads as curl. Don't "make these
        ...
        {/* Loki: the apricot one, in front and to the left. Same silhouette
            as Finn; the coat is the whole read. */}
        <g>
          {/* Drop ears, longer and shaggier since he's the fluffier of the
           ...
           *
           * Kept inside the skull's outline, which it wasn't before: it sat
           * at cy 23.4 with ry 5.4, so its top reached y 18 against a skull
           * that starts at 20.8, and it was 11 wide where the skull is 10.2.
           * That put a darker arc outside the head silhouette, a shape
           * sitting on top of a dog rather than growing out of it, reading as
           * a beret. It went unnoticed for the same reason Finn's did: a
           * visor was drawn over exactly that band, so the protrusion was
           * never visible.
           *
           * Unlike Finn's above, this one stays a shadow. Apricot has
           * somewhere darker to go, so a dark fringe on Loki reads as curl
           * once it's inside his outline; the fix here is geometry, not
           * value. */}
           ...
           *  uniform's tie is #14100c against a #123527 disc, a black shape on
           *  a dark green one, invisible at 34px and not much better at 200.
           *  The art solves it with an ivory shirt behind the tie, so this
           *  does too, which is also why `shirt` is a field on DogUniform
           *  rather than a colour hard-coded here.
           *
           *  Two numbers here answer to the disc, not taste. It sits at cy 58
           *  rather than the old 60.8 because the circle narrows fast down
           *  there: at y 60.8 it spans x 18-46, so a bow under Loki's chin
           *  lost its whole left wing, while at 58 it spans 13.4-50.7 and the
           *  bow survives. And it's centred on x 22 rather than Loki's own 21
           *  because he's off-centre in a disc centred on 32, so a shape
           *  centred on him gets clipped hard on the left and not at all on
           *  the right, reading as a collar worn crooked rather than one
           *  that's merely cropped. A one-unit nudge is invisible under the
           *  chin and squares it up.
           *
           *  Clipping is fine and intended; losing half the only uniform in
           *  the picture, or wearing it askew, is not. */}
