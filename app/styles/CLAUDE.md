...
- Palette: dark obsidian ground (`--brand-ink` `#0a0a0b`), brand purple/red/gold
  (`#983fe0`/`#dc1413`/`#db9c0b`, frozen historical values sampled from a since-replaced logo — see
  `01-tokens.css` — not the current mark's own Neon Marquee purple/yellow) reserved for chrome
  accents and single primary actions, not a wash across a whole surface. Table felt/gold
  (`05-game-header.css` through `09-action-bar.css`, plus `16`/`17`/`99`) is untouched green felt and
  out of scope for chrome work.
