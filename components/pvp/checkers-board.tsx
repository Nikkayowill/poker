 ...
 * Every legality question is answered by `state.legalMoves`, generated by the
 * engine on the server. A client-side rule set would be a second definition
 * of checkers, and the two would disagree on exactly the hard cases (forced
 * jumps, chains, crowning).
 *
 ...
 * a legal move. No legal move is a prefix of another from the same square, so
 * a chain that can continue must continue, and the match is unambiguous.
 */
 ...
 * actually runs out. A display showing 0:00 while a move is still legal is
 * the kind of thing a player reports as a lost game.
 */
...
export function CheckersBoard({ state, yourSeat, busy, onMove }: DuelBoardProps<CheckersSnapshot>) {
   ...
   * far. Not reconciled against a new snapshot by an effect: `candidates`
   * below re-derives everything from whatever the server last said, so a
   * selection that no longer describes a legal move simply stops being one
   * on the next render. An effect here would be a second source of truth for
   * a value that's already derivable.
   */
  ...
  // through 180 degrees on both axes, not just the rows. Rotating only the
  // rows would mirror the board instead of rotating it, and the diagonals
  // would read wrong.
  ...
  // Seat 0 is always red (see lib/pvp/checkers.ts); restated here only for
