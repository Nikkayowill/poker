...
import { PAINTERS } from "./homestead-art";
 ...
 * vector painters (see homestead-art.ts), so this composes a farm-at-dusk
 ...
 */
export function HomesteadCoverArt() {
  ...
  return <canvas ref={ref} className="hs-play-cover" aria-hidden="true" />;
}
...
function paintScene(c: Ctx, w: number, h: number): void {
  ...
  // The Homestead is landscape-only (homestead-farm.tsx gates everything,
  ...
  // out a stacked sky-over-ground layout: the .hs-play-content column is
  ...
  // rule every painter in homestead-art.ts is lit by, standing in here for a
