 ...
 *  age. */
export const STACKACRES_ACTIVE_MACHINES: readonly MachineKind[] = ["mill", "dairy", "oven", "stew_pot", "counter", "feed_silo"];
 ...
 *  or a seed already bagged, still grows and harvests as before. */
export const STACKACRES_RETIRED_CROPS: readonly StackAcresStock[] = ["wheatsheaf"];

/** Whether `stock` is buyable/plantable in this pass. */
export function isActiveStock(stock: StackAcresStock): boolean {
  if (isStackAcresCrop(stock)) return !STACKACRES_RETIRED_CROPS.includes(stock);
