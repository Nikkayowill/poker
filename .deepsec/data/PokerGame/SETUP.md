# Agent setup for `PokerGame`

This is a deepsec scanning workspace. Project `PokerGame` was just registered
(target: `..`). Setup is incomplete — `data/PokerGame/INFO.md`
...
2. **Fill in `data/PokerGame/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id PokerGame
pnpm deepsec process --project-id PokerGame
```
