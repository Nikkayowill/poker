 ...
 *
 * This used to also run a second, always-replayable "Ante Up: Word Stack"
 * game on a fresh (non-daily) word (see git history around 2026-08-21 if that
 * shape is ever needed again). It was cut the same day it shipped: Kayo's
 * call was that Word Stack keeps its once-a-day limit, full stop, and a wager
 * attaches to *that* one attempt instead of unlocking unlimited replay on a
 * separate board -- unlimited replay is what Sudoku/Memory Match get instead,
 * precisely because they have no daily identity worth protecting the way a
 * shareable daily word does. What is left here is pure scoring math, used by
 * lib/server/word-stack-service.ts: how much a wagered win pays
 * (anteUpWordStackPayout), and how much the free path's completion bonus pays
 * (wordStackDailyBonusMultiplier) -- the two are mutually exclusive per round,
 * see that file for the "wager replaces the bonus" rule.
 */
