# StackAcres room art: credits

The inside of the house, the barn, the workshop and the city grocery is built from **LPC Revised** by Eliza Wyatt and the
Liberated Pixel Cup artists (https://github.com/ElizaWy/LPC). Every sheet used is listed below with its
artists, licence and original source, as its own Credits.txt gives them. All are OGA-BY 3.0
(https://static.opengameart.org/OGA-BY-3.0.txt), some with CC0 originals. The sheets are vendored under
`art/stackacres-td/lpc/interior/` with the pack's folder names. The room border is recoloured to orange
and the grocery's notice board (a sign panel) to cork; nothing else is changed.

- **Objects/Furniture/Barrel.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles
- **Objects/Furniture/Beds/Beds, Double C.png**: Lanea Zimmerman (Sharm), BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-upholstery
- **Objects/Furniture/Beds/Beds, Double Headboards.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Objects/Furniture/Beds/Beds, Double Mattresses.png**: Eliza Wyatt (DeathsDarling), Lanea Zimmerman (Sharm), BlueCarrot16. OGA-BY 3.0. https://opengameart.org/content/lpc-upholstery
- **Objects/Furniture/Cabinet.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Objects/Furniture/Chest.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles
- **Objects/Furniture/Clock, Grandfather.png**: Lanea Zimmerman (Sharm). OGA-BY 3.0. https://opengameart.org/content/lpc-interior-castle-tiles
- **Objects/Furniture/Countertop.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Objects/Furniture/Crate.png**: Eliza Wyatt (DeathsDarling). OGA-BY 3.0. 
- **Objects/Furniture/End Table.png**: Eliza Wyatt (DeathsDarling). OGA-BY 3.0. 
- **Objects/Furniture/Fireplace, Cast Iron.png**: Lanea Zimmerman (aka Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Objects/Furniture/Fireplace.png**: Lanea Zimmerman (aka Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Objects/Furniture/Fridge.png**: Eliza Wyatt (DeathsDarling). OGA-BY 3.0.
- **Objects/Furniture/Lighting, Floor.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles, https://opengameart.org/content/lpc-interior-castle-tiles
- **Objects/Furniture/Planter.png**: Lanea Zimmerman (Sharm). OGA-BY 3.0. https://opengameart.org/content/lpc-interior-castle-tiles
- **Objects/Furniture/Rugs/Diamond Rug, tiling.png**: Eliza Wyatt (DeathsDarling). OGA-BY 3.0. 
- **Objects/Furniture/Sawhorse.png**: BlueCarrot16. OGA-BY 3.0. https://opengameart.org/content/lpc-woodshop
- **Objects/Furniture/Seating/Chair, Dining A.png**: Lanea Zimmerman (Sharm), BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-upholstery
- **Objects/Furniture/Seating/Chair, Sofa D.png**: Lanea Zimmerman (Sharm), BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-upholstery
- **Objects/Furniture/Seating/Loveseat, Small - Casual Solid A.png**: Lanea Zimmerman (Sharm), BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-upholstery
- **Objects/Furniture/Sewing & Weaving/Loom.png**: BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-tailor
- **Objects/Furniture/Sewing & Weaving/Spinning Wheel.png**: BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-tailor
- **Objects/Furniture/Shavehorse.png**: BlueCarrot16, Eliza Wyatt. OGA-BY 3.0. https://opengameart.org/content/lpc-woodshop
- **Objects/Furniture/Shelf.png**: Eliza Wyatt (DeathsDarling). OGA-BY 3.0. 
- **Objects/Furniture/Sink, Countertop.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Objects/Furniture/Table, Rough Wood.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-dungeon-elements
- **Objects/Furniture/Trough.png**: BlueCarrot16, Eliza Wyatt (DeathsDarling), YuriNikolai. OGA-BY 3.0. https://opengameart.org/content/lpc-blacksmith, https://opengameart.org/content/lpc-troughs-expanded
- **Objects/Furniture/Workbench, Carpentry.png**: BlueCarrot16. OGA-BY 3.0. https://opengameart.org/content/lpc-woodshop
- **Objects/Furniture/Workbench, Wire Drawing.png**: BlueCarrot16. OGA-BY 3.0. https://opengameart.org/content/lpc-blacksmith
- **Objects/Moveable/Shopping Cart.png**: Eliza Wyatt (DeathsDarling). OGA-BY 3.0.
- **Objects/Small Items/Baskets A.png**: BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-tailor
- **Objects/Small Items/Buckets.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles
- **Objects/Small Items/Dishes A.png**: Richard Kettering (Jetrel), BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0 (Original art CC0). https://opengameart.org/content/rpg-item-set, https://opengameart.org/content/lpc-meals
- **Objects/Small Items/Flowers.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-interior-castle-tiles
- **Objects/Small Items/Food/Bread B.png**: BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0 (Original art CC0). https://opengameart.org/content/lpc-food
- **Objects/Small Items/Food/Cheese A.png**: Thekingphoenix, BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0 (Original art CC0). http://opengameart.org/content/icons-food
- **Objects/Small Items/Food/Eggs.png**: BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0 (Original art CC0). https://opengameart.org/content/lpc-food
- **Objects/Small Items/Food/Fruit A.png**: BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0 (Original art CC0). https://opengameart.org/content/lpc-meals
- **Objects/Small Items/Food/Grains, Grasses.png**: BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0 (Original art CC0). https://opengameart.org/content/lpc-food
- **Objects/Small Items/Food/Vegetables A.png**: BlueCarrot16, Eliza Wyatt (DeathsDarling). OGA-BY 3.0 (Original art CC0). https://opengameart.org/content/lpc-meals
- **Objects/Small Items/Hay & Straw.png**: Lanea Zimmerman (Sharm). OGA-BY 3.0. https://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles
- **Objects/Small Items/Kitchen Clutter A.png**: Lanea Zimmerman (Sharm), Hyptosis, Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-interior-castle-tiles, https://opengameart.org/content/mage-city-arcanos
- **Objects/Small Items/Lighting, Table.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles, https://opengameart.org/content/lpc-interior-castle-tiles
- **Objects/Small Items/Lumber.png**: BlueCarrot16. OGA-BY 3.0. https://opengameart.org/content/lpc-woodshop
- **Objects/Small Items/Sawdust.png**: BlueCarrot16. OGA-BY 3.0. https://opengameart.org/content/lpc-woodshop
- **Objects/Small Items/Tools, Carpentry.png**: BlueCarrot16. OGA-BY 3.0. https://opengameart.org/content/lpc-woodshop
- **Objects/Wall Items/Curtains.png**: Lanea Zimmerman (Sharm), BlueCarrot16. OGA-BY 3.0. https://opengameart.org/content/lpc-upholstery
- **Objects/Wall Items/Lighting, Wall.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Objects/Wall Items/Paintings, Landscape.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-interior-castle-tiles
- **Objects/Wall Items/Paintings, Still Life.png**: Eliza Wyatt (DeathsDarling). OGA-BY 3.0. 
- **Objects/Wall Items/Posters.png**: Eliza Wyatt (DeathsDarling). OGA-BY 3.0.
- **Structure/Doors/64x64px Arched Doors/Arched Double Doors A.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-interior-castle-tiles
- **Structure/Floor/Wood Floor A.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Structure/Signs/Sign Backgrounds A.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles
- **Structure/Wall Borders/Plain Edged Border.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Structure/Walls/Floral Wallpaper A.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-interior-castle-tiles
- **Structure/Walls/Half-Wall Paneling A.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-interior-castle-tiles
- **Structure/Walls/Siding, Plain.png**: Eliza Wyatt (DeathsDarling). OGA-BY 3.0. 
- **Structure/Windows/Ornamental Windows A.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-modified-base-tiles
- **Structure/Windows/Ornamental Windows B.png**: Lanea Zimmerman (Sharm), Eliza Wyatt (DeathsDarling). OGA-BY 3.0. https://opengameart.org/content/lpc-arabic-elements, https://opengameart.org/content/lpc-adobe-building-set
