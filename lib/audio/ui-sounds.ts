 ...
 *
 * ## Something on the route has to have applied the mute first
 *
 * `setSoundEnabled` in ./sound-effects is a module-level flag that defaults to
 * ON, and components/poker-app.tsx is the only thing that syncs it with the
 * player's stored mute. Anything under PokerApp -- the lobby, the table, every
 * modal they mount -- is therefore free to call these directly.
 *
 * The arcade lives on its own routes (/games/*) where PokerApp is not mounted,
 * so a page there that just called these would be loud for a player who had
 * muted the app, on a screen with no control to fix it. That is the bug
 * components/arcade/use-arcade-sound.ts exists to close.
 *
 ...
 * helpers honest for that whole page -- the hook's return value is only needed
 * by callers that want to name a non-chrome effect. Call it at the route's
 * root component and treat that call as load-bearing, not decorative: delete
 * it and every button below it silently stops respecting the mute.
 */
