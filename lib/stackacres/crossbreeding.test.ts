...
describe("CROSSBREED_MATRIX", () => {
  ...
  it("breeds every hybrid from exactly one pair, across all three tracks", () => {
    const bred = CROSSBREED_MATRIX.map((entry) => entry.hybrid);
    expect([...bred].sort()).toEqual([...CROSSBREED_ITEMS].sort());
    const tracks = new Set(CROSSBREED_MATRIX.map((entry) => crossedTrack(entry.a, entry.b)));
    expect(tracks).toEqual(new Set(["crop", "mixed", "livestock"]));
  });

  it("falls in odds from crop to mixed to livestock pairings", () => {
    const chances = (track: ReturnType<typeof crossedTrack>) =>
      CROSSBREED_MATRIX.filter((e) => crossedTrack(e.a, e.b) === track).map((e) => e.chance);
    expect(Math.min(...chances("crop"))).toBeGreaterThan(Math.max(...chances("mixed")));
    expect(Math.min(...chances("mixed"))).toBeGreaterThan(Math.max(...chances("livestock")));
  });
});
...
describe("crossbreedMatrixEntryFor", () => {
  ...
  it("returns null for a pair StackAcres has never defined a cross for", () => {
    // hen + carrot is not in CROSSBREED_MATRIX above.
