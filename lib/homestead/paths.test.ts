...
import {
  ...
  HOMESTEAD_CELL,
  HOMESTEAD_MARGIN,
  HOMESTEAD_WORLD_COLUMNS,
  HOMESTEAD_WORLD_ROWS,
  cellCenter,
  inFarmZone,
} from "./world";
...
const PLOTS = {
  x: HOMESTEAD_MARGIN,
  y: HOMESTEAD_MARGIN,
  width: HOMESTEAD_WORLD_COLUMNS * HOMESTEAD_CELL,
  height: HOMESTEAD_WORLD_ROWS * HOMESTEAD_CELL,
};
...
describe("farm paths", () => {
  it("has four paths with unique keys, at least two points each, 10..20 wide", () => {
    expect(FARM_PATHS.length).toBe(4);
  ...
  it("keeps the yard paths inside the farm zone and lets the road and track leave it", () => {
    const lane = byKey("lane");
    for (const p of lane.points) expect(inFarmZone(p.x, p.y)).toBe(true);
    for (const key of ["road", "track"]) {
